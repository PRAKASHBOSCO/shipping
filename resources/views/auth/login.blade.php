						...
						<div class="login-signin">
                            ...
                            <form class="form" method="POST" role="form" action="{{ route('login') }}">
								...
								<div class="mb-5 form-group">
                                    <input id="email" type="text" class="form-control h-auto form-control-solid py-4 px-8 {{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" required autofocus placeholder="Email / Phone No.">
                                    @if ($errors->has('email'))
                            ...
                            @if(\App\BusinessSetting::where('type', 'google_login')->first()->value == 1 || \App\BusinessSetting::where('type', 'facebook_login')->first()->value == 1 || \App\BusinessSetting::where('type', 'twitter_login')->first()->value == 1)
                                <div class="mb-3 separator">
                                    <span class="px-3 bg-white opacity-60">{{ translate('Or Login With')}}</span>
                                </div>
                                <ul class="mb-5 text-center list-inline social colored">
