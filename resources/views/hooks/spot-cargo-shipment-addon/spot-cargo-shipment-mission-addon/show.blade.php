            ...
            <table class="table">
                ...
                @foreach(\App\ShipmentMission::where('mission_id',$data['mission']->id)->get() as $shipment_mission)
                    <tr class="font-weight-boldest @if(in_array($shipment_mission->shipment->status_id ,[\App\Shipment::RETURNED_STATUS,\App\Shipment::RETURNED_STOCK,\App\Shipment::RETURNED_CLIENT_GIVEN])) table-danger @endif">
                        @if(in_array($user_type ,['admin','captain']) || in_array('1100', $staff_permission) || in_array('1005', $staff_permission) )
                        ...
                        @if($shipment_mission->shipment->payment_type == \App\Shipment::POSTPAID)
                            <td class="text-right  pt-7">{{format_price($shipment_mission->shipment->amount_to_be_collected + $shipment_mission->shipment->tax + $shipment_mission->shipment->shipping_cost + $shipment_mission->shipment->insurance + $shipment_mission->shipment->pickup_cost + $shipment_mission->shipment->shipping_distance_cost + $shipment_mission->shipment->door_delivery_cost + $shipment_mission->shipment->cod_cost - $shipment_mission->shipment->discount_amt) }}</td>
                        @elseif($shipment_mission->shipment->payment_type == \App\Shipment::PREPAID)
                            ...
                            @else
                                <td class="text-right  pt-7">{{format_price($shipment_mission->shipment->tax + $shipment_mission->shipment->shipping_cost + $shipment_mission->shipment->insurance + $shipment_mission->shipment->pickup_cost + $shipment_mission->shipment->shipping_distance_cost + $shipment_mission->shipment->cod_cost + $shipment_mission->shipment->door_delivery_cost - $shipment_mission->shipment->discount_amt) }}</td>
                            @endif
                        ...
                        <td class="pr-5 text-right text-danger pt-7 no-print">
                            @if(in_array($shipment_mission->mission->status_id , [\App\Mission::APPROVED_STATUS,\App\Mission::REQUESTED_STATUS,\App\Mission::RECIVED_STATUS]) && $shipment_mission->shipment->mission_id != null)
                                ...
                                @if($data['mission']->status_id == \App\Mission::RECIVED_STATUS)
                                    ...
                                    <a class="btn btn-success btn-sm mb-1" data-url="{{route('admin.missions.action.confirm_amount', ['mission_id' => $data['mission']->id , 'shipment_id' => $shipment_mission->shipment->id ])}}" data-action="POST" onclick="openAjexedModel(this,event)" href="{{route('admin.missions.show', $mission_id)}}" title="{{ translate('Show') }}">
