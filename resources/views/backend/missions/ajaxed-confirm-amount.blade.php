        ...
        <div class="row">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    @if($mission->getOriginal('type') ==  \App\Mission::DELIVERY_TYPE  && $mission->status_id == \App\Mission::RECIVED_STATUS)
                        <input type="hidden" class="form-control" value="{{$shipment->id}}" name="shipment_id" />
                    @endif
                    ...
                    @else
                        @if($shipment->payment_type == \App\Shipment::POSTPAID)
                            <input type="number" class="form-control"  value="{{ $shipment->amount_to_be_collected + $shipment->shipping_cost + $shipment->tax + $shipment->insurance }}" name="amount"
                                style="background:#f3f6f9;color:#3f4254;" disabled /> 
                        @elseif($shipment->payment_type == \App\Shipment::PREPAID)
                            ...
                            @if($mission->getOriginal('type') == \App\Mission::DELIVERY_TYPE)
                                ...
                                style="background:#f3f6f9;color:#3f4254;" disabled /> 
                            @else
                                <input type="number" class="form-control"  value="{{ $shipment->shipping_cost + $shipment->tax + $shipment->insurance }}" name="amount"
                                style="background:#f3f6f9;color:#3f4254;" disabled /> 
                            @endif
                    ...
                    @endif
                    @else
                        <input type="number" class="form-control" value="{{$mission->amount}}" name="amount" style="background:#f3f6f9;color:#3f4254;" disabled />
                    @endif
