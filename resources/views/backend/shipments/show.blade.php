        ...
        <div class="px-8 py-8 row justify-content-center pt-md-27 px-md-0">
            <div class="col-md-10">
                <div class="pb-10 d-flex justify-content-between pb-md-20 flex-column flex-md-row">
                    ...
                    <div class="px-0 d-flex flex-column align-items-md-end">
                        <span class="d-flex flex-column align-items-md-end opacity-70">
                            ...
                            @endif
                            <span><span class="font-weight-bolder">{{translate('FROM')}}:</span> {{$shipment->from_address->address}}</span>
                ...
                <div class="pb-6 d-flex justify-content-between">
                    <div class="d-flex flex-column flex-root">
                        ...
                        @if(Auth::user()->user_type == 'admin' || in_array('1005', json_decode(Auth::user()->staff->role->permissions ?? "[]")))
                            <a class="text-danger font-weight-boldest font-size-lg" href="{{route('admin.clients.show',$shipment->client_id)}}">{{$shipment->client_id == 68 ? $shipment->walkin_customer_name : $shipment->client->name}}</a>
                        @else
                            <span class="text-danger font-weight-boldest font-size-lg">{{$shipment->client_id == 68 ? $shipment->walkin_customer_name : $shipment->client->name}}</span>
                        @endif
                        <span class="text-muted font-size-md">{{$shipment->client_phone}}</span>
                        <span class="text-muted font-size-md">{{$shipment->client_id == 68 ? $shipment->walkin_client_address : $shipment->from_address->address}}</span>
                    </div>
                    <div class="d-flex flex-column flex-root">
                ...
                <div class="pt-6 d-flex justify-content-between">
                    ...
                    <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('To Region')}}</span>
                ...
                <div class="pt-6 d-flex justify-content-between">
                    ...
                    @if($shipment->collection_time != null)
                        <div class="d-flex flex-column flex-root">
                            <span class="mb-4 text-dark font-weight-bold">{{translate('Collection Time')}}</span>
                    ...
                    @if ($shipment->mission_id != null)
                        <div class="d-flex flex-column flex-root">
                            ...
                            @if(Auth::user()->user_type == 'admin' || in_array('1008', json_decode(Auth::user()->staff->role->permissions ?? "[]")))
                                <a class="text-danger font-weight-bolder font-size-lg" href="{{route('admin.missions.show',$shipment->mission_id)}}">{{$shipment->current_mission->code}}</a>
                            @else
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr class="font-weight-bolder">
                                ...
                                <td class="text-right text-primary font-size-h3 font-weight-boldest">{{format_price($shipment->amount_to_be_collected + $shipment->tax + $shipment->shipping_cost + $shipment->insurance + $shipment->pickup_cost + $shipment->shipping_distance_cost + $shipment->door_delivery_cost + $shipment->cod_cost - $shipment->discount_amt) }}<br /><span class="text-muted font-weight-bolder font-size-lg">{{translate('Included tax & insurance')}}</span></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="card card-custom card-stretch-half gutter-b">
        ...
        <div class="pt-6 mb-2 border-0 card-header">
            <h3 class="card-title align-items-start flex-column">
                <span class="mb-3 card-label font-weight-bold font-size-h4 text-dark-75">{{translate('Shipment Return Reasons Log')}}</span>
            ...
            @forelse($shipment->shipmentReasons as $key => $shipmentReason)
                ...
                <div class="timeline-item align-items-start">
                    ...
                    <div class="pl-3 font-weight-mormal font-size-lg timeline-content text-muted">
                        {{translate('Reason '.($key+1) )}}: "{{$shipmentReason->reason->name}}"
                    </div>
                    <!--end::Text-->
