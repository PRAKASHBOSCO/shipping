                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-2">
                                <label>{{translate(' Delivery Time')}} (In Days):</label>
                                <input type="number" onchange="changeTax(this)" min="0" id="name" class="form-control" placeholder="{{translate('Days')}}" value="@php if(isset($default_country_cost->delivery_time)){echo $default_country_cost->delivery_time;}else{ echo 1;} @endphp" name="delivery_time[]">
                            </div>
                            <div class="form-group col-md-2">
...
<script>
    $('.evenAjaxButton').hide();
    function cost_block(label_name, label_returned_name, input_name, input_returned_name, from_name, to_name, from_id, to_id, from_city_name, to_city_name, from_city_id, to_city_id, shipping_cost, tax, delivery_time, return_cost, insurance) {
    return `<div class="row">
                        <div class="col-lg-12">
                            <div class="row">
                                ...
                                <div class="form-group col-md-2">
                                    <label>{{translate(' Delivery Time')}} (In Days):</label>
                                    <input type="number" min="0" id="delivery_time" class="form-control tax_cost" placeholder="{{translate('Days')}}" value="` + delivery_time + `" name="delivery_time[]">
                                </div>
                                <div class="form-group col-md-2">
                                    <label>{{translate(' Insurance')}} ({{currency_symbol()}}):</label>
    ...
    $.ajax("{{route('admin.shipments.config.costs.ajax')}}?from_country={{$from->id}}&to_country={{$to->id}}")
            ...
            $('#placeholder_cost').append(cost_block(label_shipping_cost, label_returned_cost, input_name, input_returned_name, v.from_country, v.to_country, v.from_country_id, v.to_country_id, v.from_state, v.to_state, v.from_state_id, v.to_state_id, v.shipping_cost, v.tax, v.delivery_time, v.return_cost, v.insurance));
            } else if ({{$is_def_mile_or_fees}} == '1'){
            $('#placeholder_cost').append(cost_block(label_mile_cost, label_returned_mile_cost, input_mile_name, input_returned_mile_name, v.from_country, v.to_country, v.from_country_id, v.to_country_id, v.from_state, v.to_state, v.from_state_id, v.to_state_id, v.mile_cost, v.tax, v.delivery_time, v.return_mile_cost, v.insurance));
            }
            });
