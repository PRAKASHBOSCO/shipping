...
@php
    ...
    if($user_type == 'branch'){
        $branchs = \App\Branch::where('is_archived', 0)->where('id',Auth::user()->userBranch->branch_id)->get();
    }else{
        ...
        <div class="mb-7">
            ...
            <table class="table mb-0 aiz-table">
                ...
                <tbody>
                    ...
                    @php
                    $totalShipping = $totalShipping + $shipment->shipping_cost;
                    $total_shipping = number_format($totalShipping,2);
                    $shipping_cost  = number_format($shipment->shipping_cost,2);
                    @endphp
                    ...
                    <tr>
                        ...
                        @if($user_type == 'admin' || in_array('1005', $staff_permission)  )
                            <td><a href="{{route('admin.clients.show',$shipment->client_id)}}">{{$shipment->client_id == 68 ? $shipment->walkin_customer_name : $shipment->client->name}}</a></td>
                        @else
                            <td>{{$shipment->client_id == 68 ? $shipment->walkin_customer_name : $shipment->client->name}}</td>
                        @endif
                ...
                <tfoot>
                    <tr>
                        ...
                        <td style="text-align:right;font-weight:bold;">{{$total_shipping}}</td>
                    </tr>
                </tfoot>
            </table>
